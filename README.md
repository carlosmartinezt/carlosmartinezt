...
| | | |
|---|---|---|
| **Chloe**<br>https://chloejs.org<br>[source](https://github.com/carlosmartinezt/chloejs) | A TypeScript agent framework that uses AI only where a step needs judgement. The workflow is plain code and a model is one step in it. Built on the bet that most of an agent's work should be predictable. The dashboard, [@chloejs/ui](https://www.npmjs.com/package/@chloejs/ui), shows every agent, run and memory, and also runs alone as a file browser (`npx @chloejs/ui <folder>`). It runs the agents that look after this server. | TypeScript · Node · SQLite · React |
